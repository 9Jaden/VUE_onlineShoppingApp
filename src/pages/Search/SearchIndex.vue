        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <!-- 排序的结构 -->
              <ul class="sui-nav">
                <!-- 点击事件changeOrder()需要传参来区分点的是综合还是价格 -->
...
export default {
  ...
  data() {
    ...
    return {
      searchParams: {
        ...
        categoryName: "",
        keyword: "",
        order: "1:desc", // 1代表综合 2为价格 des降序 asc升序
        pageNo: 1,
        pageSize: 3,
        props: [], //相关属性
        trademark: "", //品牌
  ...
  beforeMount() {
    ...
    // 在发请求之前，把接口需要传递参数进行整理，服务器则会返回所查询数据
  ...
  computed: {
    //mapGetters里面的写法：传递的是数组，因为getters计算没有划分模块
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      // 把带给服务器的置空后还需要再次发请求
      this.getData();
      // 地址栏也需要修改，在自己当前路由又跳回自己即可，但是注意需要保留params参数
    ...
    getAttr(id, value, name) {
      let newProps = `${id}:${value}:${name}`;
      // 如果这个newProps已经在props里有了则不再push进去，以免出现重复面包屑
  ...
  // 数据监听：监听组件实例身上的属性变化
  watch: {
    // 监听路由的信息是否发生变化，如果发生变化则再次发起请求
    $route() {
      // 再次发请求之前需要整理带给服务器参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      // 再次发起ajax请求
      this.getData();
      // 每一次请求完毕应该把相应的1、2、3级分离的id置空以接受下一次新的参数
