...
export default {
  ...
  methods: {
    ...
    async allCheckedOrNot(event) {
      try {
        let check = event.target.checked ? "1" : "0";
        // 派发action
        await this.$store.dispatch("allCheckedOrNot", check);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
