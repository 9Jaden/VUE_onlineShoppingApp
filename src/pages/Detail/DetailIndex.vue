        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转，从A到B，而这里在路由跳转之前还发了请求，把购买的产品的信息通过请求的形式通知服务器进行存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      //派发action，将产品加入到数据库，通知服务器，注意用对象形式传参

      // 调用函数返回promise可知成功或失败
      //服务器存储成功进行路由跳转
      //服务器存储失败给用户进行提示
      try {
        ...
        // 在路由跳转时还需要把产品的信息带给下一级的路由组件，详情信息（对象）由会话存储传输
        // 本地存储|会话存储，一般存储的是字符串
